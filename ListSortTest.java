 ...
 * 
 * - Comparable 에서는 CompareTo() 메서드를 구현해야 하고, Comparator에서는 compare() 메서드를 구현해야 한다.
 */
...
public class ListSortTest {
	public static void main(String[] args) {
		...
		// 정렬방식을 변경하려면 정렬방식을 결정하는 객체를 만들어서 Collections.sort() 메서드에 
		// 인수로 넘겨주면 된다.
...
// 정렬방식을 결정하는 class는  Comparator 라는 인터페이스를 구현해야 된다.
// 이 Comparator 인터페이스의 compare() 라는 메서드를 재정의하여 구현하면 된다.
	 ...
	 * 
	 * - 오름차순 정렬일 경우
	 * => 앞의 값이 크면 양수, 같으면 0, 앞의 값이 작으면 음수를 반환 하도록 한다.
	 * 
...
class Desc implements Comparator<String>{
	...
	public int compare(String str1, String str2) {
		return str1.compareTo(str2) * -1; //1을 '-1'로 바꾸면 오름차순에서 내림차순으로 바뀐다.
										  // 양수값은 음수로, 음수는 양수로 바뀌니까
	}
}