...
public class SetTest {
	public static void main(String[] args) {
		...
		// Set 은 데이터의 순서가 없고, 중복을 허용하지 않는다.
		// 그래서 이미 있는 데이터를 add 하면 false 를 반환하고, 데이터는 추가되지 않는다.
		...
		// Set 의 데이터를 수정하려면 수정하는 명령이 따로 없기 때문에 해당 자료를 삭제한 후 새로운 데이터를 추가해 줘야 한다.
		...
		// Set 은 데이터의 순서가 없기 때문에 List 처럼 인덱스로 데이터를 하나씩 불러올 수 없다.
		// 그래서 데이터를 하나씩 얻기 위해서는 Iterator 로 변환해야 한다.
		
		// Set의 데이터를 Iterator 로 변환하기 => Set 의 Iterator() 메서드를 호출하면 된다.
		Iterator it = hs1.iterator();
		 
		// 데이터 개수만큼 반복하기
		// hasNext() 메서드 => 포인터를 다음 위치에 옮기고, 데이터가 잆으면 true, 없으면 false 를 반환한다.
		...
		while(intRnd.size() < 5) {	// Set 의 데이터가 5개 될 때까지 반복...
			int num = (int)(Math.random() * (100) + 1 ); // 1~100 사이 난수
			intRnd.add(num);
		}
		...
		System.out.println("List 의 자료 출력...");
