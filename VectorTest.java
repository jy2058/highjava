...
public class VectorTest {
	public static void main(String[] args) {
		...
		Vector v1 = new Vector(); //list 로 바꿔도 에러나지 않음
		...
		v1.add("aaa");
		v1.add(111);
		v1.add(new Integer(111)); //얘는 객체이기 때문에(래퍼 클래스)(박싱) 위의 것과는 다른 것이다.
		v1.add('a');
		v1.add(true);
		v1.add(3.14);	// 오토박싱 적용돼서 오류가 나지 않음
		...
		v1.addElement("ccc");	//벡터만 가지고 있는 메서드
		...
		// removAll(Collection 객체) => 벡터의 데이터 중에서 'Collection' 객체가 가지고 있는 데이터를 모두 삭제한다.
		v2.removeAll(v4);
		 ...
		 * 		}
		 * 
		 * => 주어진 '배열변수나 Collection 계열 변수'의 데이터 개수만큼 반복한다.
		 * => 반복할 때마다 '배열변수나 Collection계열 변수'의 데이터를 하나씩 꺼내와 '변수명'에 저장한 후 '처리할 내용 들'을 처리한다.
		 * 
		 */
		for(String s : v2) {
